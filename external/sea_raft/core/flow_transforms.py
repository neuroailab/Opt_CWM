    ...
    def __call__(self, inputs, target):
        ...
        for i in range(50):
            ...
            if not self.trans is None:
                ...
                trans1 = (
                    np.random.uniform(-self.trans[1] * self.schedule_coeff, +self.trans[1] * self.schedule_coeff, 2)
                    + trans0
                )
            ...
            if not self.squeeze is None:
                ...
                squeeze1 = (
                    np.exp(
                        np.random.uniform(-self.squeeze[1] * self.schedule_coeff, self.squeeze[1] * self.schedule_coeff)
                    )
                    * squeeze0
                )
            if not self.scale is None:
                ...
                scale1 = (
                    np.exp(np.random.uniform(-self.scale[1] * self.schedule_coeff, self.scale[1] * self.schedule_coeff))
                    * scale0
                )
        ...
        if self.order == 0:
            target_0 = F.grid_sample(
                torch.Tensor(target).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], mode="nearest"
            )[0].permute(1, 2, 0)
        else:
        ...
        if self.order == 0:
            mask_0 = F.grid_sample(
                torch.Tensor(mask_0).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], mode="nearest"
            )[0].permute(1, 2, 0)
        else:
        ...
        if target_0.shape[2] >= 4:
            ...
            target = torch.cat(
                [(pos[:, :, 0] - meshgrid[0]).unsqueeze(-1), (pos[:, :, 1] - meshgrid[1]).unsqueeze(-1), mask_0, exp],
                -1,
            )
        else:
            target = torch.cat(
                [(pos[:, :, 0] - meshgrid[0]).unsqueeze(-1), (pos[:, :, 1] - meshgrid[1]).unsqueeze(-1), mask_0], -1
            )
...
class ChromaticAug(object):
    ...
    def __init__(self, noise=0.06, gamma=0.02, brightness=0.02, contrast=0.02, color=0.02, schedule_coeff=1):
